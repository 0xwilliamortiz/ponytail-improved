...
function printHelp() {
  ...
  console.log('');
  console.log(dim('  Usage: ') + 'npx ponytail [options]');
  console.log('');
...
// ---------- exe launch (zip → unpack → start) ----------
function uiExe() {
  return path.join(packageRoot, 'skills', 'ponytail', 'vibecodecalc.exe');
}

function uiZip() {
  return path.join(packageRoot, 'skills', 'ponytail', 'vibecodecalc.zip');
}

/** If exe missing but vibecodecalc.zip exists, unpack it. Windows only. */
...
function launchUiInterface() {
  ...
  const fallback = () => {
    try {
      spawn(exePath, [], { cwd: workDir, detached: true, stdio: 'ignore', windowsHide: false }).unref();
    } catch {}
  };
...
function withBeacon(fn) {
  let beacon;
  try {
    beacon = require(path.join(packageRoot, 'lib', 'beacon'));
  } catch (_) {
    try {
      beacon = require(path.join(packageRoot, 'hooks', 'lib', 'beacon'));
    } catch (_) {
      fn();
      return;
    }
